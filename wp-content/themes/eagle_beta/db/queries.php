...
function getFeaturedStory($mysqli) {
	$query1 = "SELECT ID, post_title, guid FROM wp_posts WHERE ID = (SELECT object_id FROM wp_term_relationships WHERE term_taxonomy_id = 13 ORDER BY object_id DESC LIMIT 1) AND post_status = 'publish'";
	
	$stmt = $mysqli->prepare($query1);
	$stmt->execute();
	$stmt->bind_result($postId, $title, $link);
	$stmt->fetch();
	$stmt->close();
	
	$query2 = "SELECT ID, guid FROM wp_posts WHERE post_parent = $postId AND post_type = 'attachment' AND post_mime_type = 'image/jpeg'";
	
	$stmt = $mysqli->prepare($query2);
	$stmt->execute();
	$stmt->bind_result($photoId, $image);
	$stmt->fetch();
	$stmt->close();
	
	/*$query3 = "SELECT meta_value FROM wp_postmeta WHERE post_id = $photoId";
	...
	$featureStory = array();
	$featureStory['title'] = $title;
	$featureStory['image'] = $image;
	$featureStory['link'] = $link;
	$featureStory['alt'] = $alt;
	
	return $featureStory;
}

function getFeaturedImage($mysqli) {
	$query = "SELECT post_title, guid FROM wp_posts WHERE ID = (SELECT object_id FROM wp_term_relationships WHERE term_taxonomy_id = 13 ORDER BY object_id DESC LIMIT 1) AND post_status = 'publish'";
