...
function checkRegistration(form, email, password, confirmPassword) {
    ...
    if (!regEx.test(password.value)) {
		alert("Passwords must contain at least one uppercase letter, one lowercase letter, and one number.");
        return false;
    }*/
...
function resetPassword(form, email) {
	...
	if(uppercaseCheck > 0 && lowercaseCheck > 0 && numberCheck > 0) {	
		var p_reset = document.createElement("input");
 
		// Add the new element to form
		form.appendChild(p_reset);
		p_reset.name = "p_reset";
		p_reset.type = "hidden";
		p_reset.value = tempPassword;
